...
double MeshTools::CotanWeight(const Mesh& mesh, const OpenMesh::HalfedgeHandle& heh)
{
	auto v_from = mesh.from_vertex_handle(heh);
	auto v_to = mesh.to_vertex_handle(heh);
	auto v_pre = mesh.to_vertex_handle(mesh.ccw_rotated_halfedge_handle(heh));
	auto v_next = mesh.to_vertex_handle(mesh.cw_rotated_halfedge_handle(heh));
	OpenMesh::Vec3d e_pf = mesh.point(v_from) - mesh.point(v_pre);
	OpenMesh::Vec3d e_pt = mesh.point(v_to) - mesh.point(v_pre);
	OpenMesh::Vec3d e_nf = mesh.point(v_from) - mesh.point(v_next);
	OpenMesh::Vec3d e_nt = mesh.point(v_to) - mesh.point(v_next);
	double cot_p = (e_pf | e_pt) / (e_pf % e_pt).norm();
	double cot_n = (e_nf | e_nt) / (e_nf % e_nt).norm();
	double weight = 0.5 * (cot_p + cot_n);
	return weight;
