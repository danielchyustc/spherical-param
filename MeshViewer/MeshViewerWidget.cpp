...
#include "MeshEdit/Parameterization/Spherical/SAQP.h"
#include "MeshEdit/Parameterization/Spherical/SSLIMRight.h"
#include "MeshEdit/Parameterization/Spherical/SSLIMLeft.h"
#include "MeshEdit/Parameterization/Spherical/SAPN.h"
#include "MeshEdit/Parameterization/Spherical/SAKVF.h"
#include "MeshEdit/Parameterization/Spherical/HybridSolver.h"
...
void MeshViewerWidget::UpdatePosition(void)
{
	ptMin[0] = ptMin[1] = ptMin[2] = DBL_MAX;
	ptMax[0] = ptMax[1] = ptMax[2] = -DBL_MAX;
	for (const auto& vh : mesh.vertices())
...
void MeshViewerWidget::InitEditor(void)
{
	editor = new MeshEditor(shared_ptr<Mesh>(&mesh));
}

void MeshViewerWidget::InitMinSurf(void)
{
	editor = new MinSurf(shared_ptr<Mesh>(&mesh));
}

bool MeshViewerWidget::InitParam(void)
{
	cout << "aaaa" << endl;
	if (MeshTools::HasBoundary(mesh)) param = new Progressive(shared_ptr<Mesh>(&mesh));
	else param = new HybridSolver(shared_ptr<Mesh>(&mesh));
	return MeshTools::HasBoundary(mesh);
}
...
void MeshViewerWidget::EditorRun(void)
{
	editor->Run();
}

void MeshViewerWidget::EditorReset(void)
{
	editor->Reset();
}

void MeshViewerWidget::ParamInit(void)
{
	param->Run();
	UpdatePosition();
}

void MeshViewerWidget::ParamIter(void)
{
	param->Iterate();
	UpdatePosition();
}

void MeshViewerWidget::ParamITD(void)
{
	param->IterateTillDone();
	UpdatePosition();
}

void MeshViewerWidget::ParamReset(void)
{
	param->Reset();
	UpdatePosition();
}

void MeshViewerWidget::ChangeParamViewMode(QString mode)
{
	if (mode == "Mesh") param->setViewMode(kMesh);
	else if (mode == "Plane") param->setViewMode(kPlane);
	else if (mode == "Sphere") param->setViewMode(kSphere);
	UpdatePosition();
}

void MeshViewerWidget::ChangeParamInitMethod(QString method)
{
	if (method == "Mesh Voronoi") param->setInitMethod(kMeshVoronoi);
	else if (method == "Hier Cluster") param->setInitMethod(kHierCluster);
}

void MeshViewerWidget::ChangeParamPlanarMethod(QString method)
{

}

void MeshViewerWidget::ChangeParamSphericalMethod(QString method)
{
	if (method == "Gauss Map") param = new GaussMap(param);
	else if (method == "SGD") param = new SAQP(param);
	else if (method == "SLIM") param = new SSLIMLeft(param);
	else if (method == "AKVF") param = new SAKVF(param);
	else if (method == "APN") param = new SAPN(param);
	else if (method == "Hybrid Solver") param = new HybridSolver(param);
}

void MeshViewerWidget::ChangeParamWeight(QString weight)
{
...
void MeshViewerWidget::DrawFlat(void) const
{
	...
	for (const auto& fh : mesh.faces())
	{
		...
		{
			glTexCoord3dv(mesh.texcoord3D(fvh).data());
