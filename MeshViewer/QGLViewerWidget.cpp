...
{
	QRect rect = QGuiApplication::primaryScreen()->availableVirtualGeometry();
...
void QGLViewerWidget::SetMaterial(const MaterialType& mattype)
{
...
const QGLViewerWidget::MaterialType& QGLViewerWidget::GetMaterialType(void) const
{
	return materialtype;
}
...
void QGLViewerWidget::LoadTexture2D()
{
	texture = new QOpenGLTexture(QImage("Textures/texture.png").mirrored());
...
void QGLViewerWidget::LoadTextureCubeMap()
{
	//注释变量类型，
//	QMap<QString, QString> paths; //存储cubemap六个面的纹理路径   
	QImage posX = QImage("Textures/r.png").convertToFormat(QImage::Format_RGB888);
	//Right，默认读取的纹理为32位RGB，不符合CubeMap的要求，必须转为24位RGB。   
	QImage negX = QImage("Textures/r.png").convertToFormat(QImage::Format_RGB888); //Left   
	QImage posY = QImage("Textures/g.png").convertToFormat(QImage::Format_RGB888); //Top   
	QImage negY = QImage("Textures/g.png").convertToFormat(QImage::Format_RGB888); //Bottom  
	QImage posZ = QImage("Textures/b.png").convertToFormat(QImage::Format_RGB888); //Front   
	QImage negZ = QImage("Textures/b.png").convertToFormat(QImage::Format_RGB888); //Back    
	//注释变量类型，
	texture = new QOpenGLTexture(QOpenGLTexture::TargetCubeMap);
	texture->setSize(posX.width(), posX.height(), posX.depth());
	//这个我猜测 是确定一面纹理的尺寸，然后allocate分配函数，根据TargeCubeMap，分配六面纹理的空间   
	...
	//Qt 显示不支持TargetCubeMap纹理类型R方向的扩展，不知道为什么，注释掉这个语句，一样可以正常运行 
}
