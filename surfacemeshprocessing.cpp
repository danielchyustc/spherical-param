...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actDefaultMaterial = new QAction(tr("Default Material"), this);
	actDefaultMaterial->setShortcut(tr("d"));
	actDefaultMaterial->setStatusTip(tr("Switch default material"));
	actDefaultMaterial->setCheckable(true);
	connect(actDefaultMaterial, SIGNAL(triggered()), viewer, SLOT(SwitchDefaultMaterial()));

	actGold = new QAction(tr("Gold"), this);
	actGold->setShortcut(tr("g"));
	actGold->setStatusTip(tr("Switch gold"));
	actGold->setCheckable(true);
	connect(actGold, SIGNAL(triggered()), viewer, SLOT(SwitchGold()));

	actSilver = new QAction(tr("Silver"), this);
	actSilver->setShortcut(tr("s"));
	actSilver->setStatusTip(tr("Switch silver"));
	actSilver->setCheckable(true);
	connect(actSilver, SIGNAL(triggered()), viewer, SLOT(SwitchSilver()));

	actEmerald = new QAction(tr("Emerald"), this);
	actEmerald->setShortcut(tr("e"));
	actEmerald->setStatusTip(tr("Switch emerald"));
	actEmerald->setCheckable(true);
	connect(actEmerald, SIGNAL(triggered()), viewer, SLOT(SwitchEmerald()));

	actTin = new QAction(tr("Tin"), this);
	actTin->setShortcut(tr("t"));
	actTin->setStatusTip(tr("Switch tin"));
	actTin->setCheckable(true);
	connect(actTin, SIGNAL(triggered()), viewer, SLOT(SwitchTin()));

	QActionGroup* agMaterialGroup = new QActionGroup(this);
	agMaterialGroup->addAction(actDefaultMaterial);
	agMaterialGroup->addAction(actGold);
	agMaterialGroup->addAction(actSilver);
	agMaterialGroup->addAction(actEmerald);
	agMaterialGroup->addAction(actTin);
	actGold->setChecked(true);
	...
	actEditor = new QAction("Editor", this);
	actEditor->setStatusTip(tr("Mesh Editor"));
	connect(actEditor, SIGNAL(triggered()), viewer, SLOT(LoadEditorTab()));

	actMinSurf = new QAction("Minimal Surface", this);
	actMinSurf->setStatusTip(tr("Compute minimal surface for mesh with boundaries"));
	connect(actMinSurf, SIGNAL(triggered()), viewer, SLOT(LoadMinSurfTab()));

	actParam = new QAction("Parameterization", this);
	actParam->setStatusTip(tr("Compute parameterization and texture coordinates"));
	connect(actParam, SIGNAL(triggered()), viewer, SLOT(LoadParamTab()));
...
void SurfaceMeshProcessing::CreateMenus(void)
{
	...
	QMenu* menuMaterial = menuView->addMenu(tr("Material"));
	menuMaterial->addAction(actDefaultMaterial);
	menuMaterial->addAction(actGold);
	menuMaterial->addAction(actSilver);
	menuMaterial->addAction(actEmerald);
	menuMaterial->addAction(actTin);
	...
	menuEdit->addAction(actEditor);
	menuEdit->addAction(actMinSurf);
	menuEdit->addAction(actParam);
